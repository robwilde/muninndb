...
func (s *Server) handleMCPInfo(w http.ResponseWriter, r *http.Request) {
	// Server-configured external URL takes precedence — set at startup from
	// MUNINN_MCP_EXTERNAL_URL for static reverse-proxy deployments.
	if s.mcpExternalURL != "" {
	...
	// MUNINN_MCP_URL is a per-request env override for deployments where the
	// listen address differs from the client-facing address (Docker, remote).
	...
	// SplitHostPort handles all valid net.Listen forms: ":port", "0.0.0.0:port",
	// "host:port", and IPv6 bracket notation "[::1]:port".
	host, port, err := net.SplitHostPort(addr)
	if err != nil {
		host = "127.0.0.1"
		port = "8750"
	}
	// Wildcard listen addresses ("", "0.0.0.0", "::") mean any interface;
	// use 127.0.0.1 to avoid IPv6 dual-stack ambiguity.
